...
function standbyActivate() {
  ...
  if (heroOrb.dataset.state === "listening") {
    handsFree = false; // kosketus keskellä kuuntelua pysäyttää koko kierron
    if (toggleMic) toggleMic();
    return;
  }
  if (toggleMic) {
    handsFree = true; // ensimmäinen kosketus käynnistää käsivapaan kierron
    toggleMic();
  } else {
...
if (Recognition) {
  ...
  rec.onerror = (e) => {
    handsFree = false; // ei jatketa loputtomiin jos esim. lupa evätty
    setOrb("idle");
    const reason = e && e.error ? e.error : "tuntematon";
    if (reason === "not-allowed" || reason === "service-not-allowed") {
      alert("Mikrofoni on estetty tälle sivulle. Korjaa: osoiterivin \"aA\" " +
            "→ Website Settings → Microphone → Allow, ja lataa sivu uudelleen.");
    } else {
      alert("Mikrofonivirhe: " + reason);
    }
  };
